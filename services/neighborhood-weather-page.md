...
Inspired by the NPS design system — black bands, colored overbars, structured grids — but with readable, traditional typography for content. **NPS elements for structure, normal text for content.**

### Fonts

Sans-serif (`--font-np`) for structural elements (headers, labels, buttons). Serif (`--font-serif`) for content (titles, descriptions, body text). This follows the actual NPS split: Frutiger for identity, NPS Rawlinson for reading. We use Lora as the Rawlinson substitute.

| Font File | Notes |
|---|---|
| `frutiger.woff2` | Primary sans — current NPS brochure typeface (2001+) |
| `nationalpark.woff2` | Display face inspired by NPS signage |
| `nps1935.woff2` | CCC-era hand-carved sign style |
| `nps-signage-1945.woff2` | Pre-Interstate highway signage |
| Cabin / Lora (Google) | Fallbacks for Frutiger / Rawlinson |

### Type Scale

| Role | Size | Weight |
|---|---|---|
| Site title | `1.9rem` | 700 |
| Data value | `1.65rem` | 700 |
| Card/content title | `1.15–1.2rem` | 700 |
| Section header bar | `1.1rem` | 700 |
| Body text | `0.9–0.95rem` | 400 |
| Label / metadata | `0.8–0.85rem` | 600 |
| Eyebrow / category | `0.68–0.72rem` | 600–700 |
| Badge / micro | `0.5–0.62rem` | 700 |

### Text Rules

**Uppercase**: structural labels only — section headers, eyebrows, category tags, status pills, buttons, weather stat labels. **Normal case**: everything the user reads — titles, descriptions, links, metadata. Never uppercase content text. This matches NPS practice (park names are title case on brochures, not all-caps).

**Letter-spacing**: wide (`0.18–0.22em`) for eyebrows/badges, medium (`0.1–0.12em`) for section headers/buttons, none for everything else. Never letter-space serif or body text.

**Colors**: warm palette only. Primary `#000`/`#333`, secondary `#4a4035`, tertiary `#6a5a4a`/`--nps-brown-light`. Avoid neutral grays (#666, #888) — they clash with the warm browns.

**Overbar colors**: green = Home & Automation, copper = Media, brown = Documents & Files, blue = Infrastructure.

### References

- [Unigrid Design Specs (PDF)](https://npshistory.com/brochures/unigrid.pdf) — original 1977 Vignelli system
- [NPS Graphic Identity](https://www.nps.gov/subjects/hfc/nps-graphic-identity-and-style-guides.htm) — Harpers Ferry Center
- [NPS Rawlinson Roadway](https://en.wikipedia.org/wiki/NPS_Rawlinson_Roadway) — typeface history
- [UniGuide Standards Manual (PDF)](https://cd3abd6beebec142023d-31d81c9257c2834bed6081c9f3253cbd.ssl.cf2.rackcdn.com/custom-signs/nps/national-park-service-uniguide-standards.pdf) — sign/color specs
- [National Park Typeface](https://nationalparktypeface.com/) — the display face we use
