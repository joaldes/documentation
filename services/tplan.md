...
- **Public hostname `trips.1701.me` LIVE** — Authentik forward-auth at the NPM edge gates the editor; `/s/*` + pretty `/<slug>` URLs bypass auth (read-only share viewer). NPM proxy host #81 forwards to `192.168.0.179:8084` with LE cert #24 (HTTP-01, auto-renew). Authentik Application `tplan` + Proxy Provider in forward-single mode, cookie domain `1701.me` for SSO across `*.1701.me`. Bound to the embedded outpost. `trip.1701.me` (old hostname, NPM host #79) is now a permanent 301 redirect — existing share links keep working. End-to-end verified: `/` → 302 Authentik login → editor; `/api/*` → bare 401; `/s/<slug>` → 200.
- **`share_mode_guard` middleware updated** — the old logic treated `SHARE_HOST` as "anonymous share viewer hostname". After consolidating editor + viewer onto the same hostname, the middleware now distinguishes via the `X-Authentik-Username` header that NPM injects on authenticated requests only. Anonymous traffic on `SHARE_HOST` still gets the share-only lockdown; authenticated traffic gets the full editor.
...
| ~~No auth on `/api/`~~ | DONE 2026-06-02 | Authentik forward-auth at NPM edge for `trips.1701.me` (NPM host #81). Path-exempt: `/s/*`, `/s/{slug}/data`, `/s/{slug}/meta`, pretty `/<slug>`. Outpost `proxy_pass`: `http://192.168.0.179:9000/outpost.goauthentik.io`. Cookie domain `1701.me` (SSO across all `*.1701.me`). LE cert #24. End-to-end verified. |
