...
| [gpu-vram-monitoring.md](services/gpu-vram-monitoring.md) | Per-container GPU VRAM monitoring for foundry (CT200) / t5 (1660 Super) — t5-host collector → Prometheus/Grafana dashboard `gpu-foundry` + >90% alert rule (Grafana UI only, no notification channel) |
...
| [seedbox-syncthing.md](services/seedbox-syncthing.md) | Seedbox → home Syncthing tuning — Ultra.cc media sync: removed the seedbox rate-limit cap (the real ~20 MiB/s bottleneck), NAT-PMP static port-forward fix, scan-delay fix |
